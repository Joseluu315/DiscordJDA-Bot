    ...
    public void execute(CommandContext ctx) throws Exception {
        ...
        if (ctx.getMember().hasPermission(Permission.BAN_MEMBERS)){
            for (Member member : ctx.getEvent().getMessage().getMentionedMembers()){
                member.ban((Integer.parseInt(args[2])), Arrays.toString(new String[]{args[2]})).queue();
                ctx.getChannel().sendMessage("Member " + member.getId() + " has been banned by operator.").queue();
            }
        }
    }
}
